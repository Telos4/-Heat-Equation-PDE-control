...
\label{eq:pde}
\end{equation}
where $y : Q \rightarrow \mathbb{R}$ is the temperature, $\alpha : Q \rightarrow \mathbb{R}$ is the diffusion coefficient. We use the shorthand $y_t = \frac{\partial y}{\partial t}$ to denote the time derivative.
...
\begin{equation}
\int_{\Omega} \frac{y_{k+1} - y_k}{h} v \; dx -  \int_{\Omega} \alpha \Delta y_{k+1} v \; dx = 0
\end{equation}
Using partial integration on the second integral we get
\begin{equation}
\int_{\Omega} \frac{y_{k+1} - y_k}{h} v \; dx - \int_{\Gamma} \alpha \frac{\partial y_{k+1}}{\partial n} v \; ds + \int_{\Omega} \alpha \nabla y_{k+1} \cdot \nabla v \; dx = 0
\end{equation}
Substituting the boundary condition \eqref{eq:robin-bc} we obtain
\begin{equation}
\int_{\Omega} \frac{y_{k+1} - y_k}{h} v \; dx +  \int_{\Gamma} \frac{\alpha \gamma}{\beta}  (y_{k+1} - z) v \; ds + \int_{\Omega} \alpha\nabla y_{k+1} \cdot \nabla v \; dx = 0
\end{equation}
...
\begin{equation}
\underbrace{
\int_{\Omega} y_{k+1} v \; dx + h \int_{\Gamma} \frac{\alpha \gamma}{\beta} y_{k+1} v \; ds + h \int_{\Omega} \alpha \nabla y_{k+1} \cdot \nabla v \; dx
}_{a(y_{k+1}, v)}
 = 
\underbrace{ 
\int_{\Omega} y_k v \; dx + h  \int_{\Gamma} \frac{\alpha \gamma}{\beta} z v \; ds
}_{F(v)}
\end{equation}
